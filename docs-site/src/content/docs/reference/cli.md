...
| `toolchain select` | Requires an AROS checkout; preview then token-confirmed atomic selection of one complete TOML v1 release lock; supports absolute `--release-lock FILE`, optional `--store DIR`, `--apply TOKEN`, and `--format human\|json` |
