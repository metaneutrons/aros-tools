//! Verified staging for Raspberry Pi SD boot bundles.
//!
...
pub const RAW_IMAGE_FILENAME: &str = "aros-pi-boot.img";
...
fn validate_required_profile_files(
    ...
    if board.transport != UBOOT_USB_ECM_TRANSPORT {
        return Ok(());
    }
    if board.model != "rpi4" {
        miette::bail!(
            "boot-bundle format 1 currently defines '{}' only for model 'rpi4'; add a new reviewed format before staging another model.",
            UBOOT_USB_ECM_TRANSPORT
        );
    }
    ...
    for (role, destination) in REQUIRED_RPI4_UBOOT_FILES {
        match by_role.get(role) {
            Some(file) if portable_path(&file.destination) == destination => {}
            Some(file) => {
                miette::bail!(
                    "Pi 4 U-Boot USB-ECM role '{}' must stage as '{}', not '{}'.",
                    role,
                    destination,
...
fn stable_volume_id(bundle: &ValidatedBootBundle) -> u32 {
    let mut hasher = Sha256::new();
    hasher.update(b"aros-pi-sd-volume-id-v1\n");
...
mod tests {
    use super::{
        ...
        BOOT_PAYLOAD_DIRECTORY, RAW_IMAGE_FILENAME, UBOOT_USB_ECM_TRANSPORT,
    };
    ...
    fn stages_a_verified_bundle_with_deterministic_metadata() {
        ...
        let checksums = fs::read_to_string(first.artifact_dir().join(ARTIFACT_CHECKSUMS))
