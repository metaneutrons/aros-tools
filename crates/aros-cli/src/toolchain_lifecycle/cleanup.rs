//! Preview-gated removal and garbage collection for managed imports.
//!
//! This module owns the destructive lifecycle boundary. It deliberately shares
//! the build-lease types from its parent rather than reconstructing ownership
//! from paths or process identifiers.

use super::{
    acquire_store_lock, directory_entry_names_nofollow_bounded, envelope_lock_path,
    is_lower_sha256, is_publication_journal_lock_name, lease_lock_path,
    managed_import_envelope_for_payload, managed_import_limits, management_store,
    measure_regular_file_bounded, measure_tree_content_cas_bounded, normalized_absolute_utf8,
    probe_advisory_file_lock, project_lock_path, project_reference_path, publication_error,
    publication_journal_lock_path, publication_journal_path, read_project_reference_unbound,
    read_registration, safe_segment, sha256_bytes, stable_token, toolchain, AdvisoryFileLock,
    AdvisoryLockState, Args, ArosToolchainLock, AtomicFilePolicy, BTreeSet, BuildLeaseReceipt,
    CommitState, Deserialize, FileIdentity, IntoDiagnostic, Path, PathBuf, Result, Serialize,
    TreeContentCas, WrapErr, LEASES_DIRECTORY, LEASE_SCHEMA, LIFECYCLE_SCHEMA,
...
mod tests {
    ...
    fn locked_build_derives_a_persistent_project_reference_instead_of_an_orphan_guard() {
        ...
        let lease = super::super::acquire_for_build_in_store(
            &project,
