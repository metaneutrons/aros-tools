---
name: aros-rpi-debug
description: Diagnose and iterate on AROS Raspberry Pi 3/4/5 bring-up through the repository's `aros board` workflow. Use for boot, deploy, UART, symbol, JTAG, SWD, TFTP, or U-Boot USB-ECM issues; not for unrelated AROS builds or general Raspberry Pi OS setup.
---
...
1. Identify the physical board and selected transport. For USB-ECM, first run the read-only `aros board scan`; pair a board by USB VID:PID, USB serial and the configured Pi-side gadget MAC, never by its transient `enX`/`enx…` name. If no local profile exists, `aros board init --profile <name> --model <model>` only prints a typed template; the local name does not select hardware. Add `--transport uboot-usb-ecm` only for Pi 4, and add `--apply` only to create a new configuration file and never to overwrite one. Run `aros board doctor` before a deployment or debugger action.
...
- Pi 3B+ and Pi 5 now have model-specific CMake and artifact contracts, but that software milestone does not prove a physical boot. Require captured UART evidence before calling either board boot-validated.
- Pi 4 alone has the reviewed `uboot-usb-ecm` transport. Pi 3 and Pi 5 use the native path; never infer USB-ECM support from their presence in the board registry.
