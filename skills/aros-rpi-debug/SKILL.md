...
1. Identify the physical board and selected transport. For USB-ECM, first run the read-only `aros board scan`; pair a board by USB VID:PID, USB serial and the configured Pi-side gadget MAC, never by its transient `enX`/`enx…` name. If no local profile exists, `aros board init --board <name> --model <model>` only prints a typed template; the local name does not select hardware. Add `--transport uboot-usb-ecm` only for Pi 4, and add `--apply` only to create a new configuration file and never to overwrite one. Run `aros board doctor` before a deployment or debugger action.
