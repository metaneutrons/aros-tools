---
name: aros-rpi-debug
description: Diagnose and iterate on AROS Raspberry Pi 4/5 bring-up through the repository's `aros pi` workflow. Use for boot, deploy, UART, symbol, JTAG, SWD, TFTP, or U-Boot USB-ECM issues; not for unrelated AROS builds or general Raspberry Pi OS setup.
---
...
1. Identify the physical board and selected transport. For USB-ECM, first run the read-only `aros pi scan`; pair a board by USB VID:PID, USB serial and the configured Pi-side gadget MAC, never by its transient `enX`/`enx…` name. If no local profile exists, `aros pi init --board <name>` only prints a template; add `--apply` only to create a new configuration file and never to overwrite one. Run `aros pi doctor` before a deployment or debugger action.
...
- `aros pi scan` is read-only candidate discovery on macOS and Linux. It reports a dynamic interface name only for diagnosis; it must not rewrite a board profile or silently choose a candidate. `aros pi serve` accepts exactly one complete paired identity before it starts the restricted DHCP/TFTP service.
...
- `aros pi sd image` never builds or downloads U-Boot. It requires a versioned external boot bundle whose board, USB identity, complete payload and hashes match the selected profile. The remaining U-Boot source/patch/`arosboot` work is therefore an explicit external-input blocker, not a reason to fabricate an SD image.
